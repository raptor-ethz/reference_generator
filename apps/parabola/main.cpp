...
void discretize_parabola(std::vector<std::vector<double>> &discretization,
                         const int gridPoints, const double a, const double b,
                         const double c) {
  // z = a * x * x + b * x + c
...
int main() {
  ...
  const int gridPoints = 80;  // -> 6second per circle
  ...
  for (int i = 0; i < gridPoints; ++i) {
    quad.go_to_pos(parabola.at(0).at(i), 1, parabola.at(1).at(i), 0, 50, true);
